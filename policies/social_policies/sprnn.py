...
from policies.social_policies.base_social_policy import BaseSocialPolicy
from utils.common import Config, dotdict
from gym.gym import Agent

class SprnnPolicy(BaseSocialPolicy):
    ...
    def __init__(self, config: Config, logger, device: str) -> None:
        super().__init__(config=config, logger=logger, device=device)
        self.setup()

    def setup(self) -> None:
        self.num_predictions = self.config.SOCIAL_POLICY["num_predictions"]

        model_path = self.config.SOCIAL_POLICY["model_config_path"]
        ...
        model_file = self.config.SOCIAL_POLICY["model_path"]
